...
router.get('/user/:telegramId/history', adminCheck, async (req, res) => {
  try {
    ...
    // Get trades (orders)
    const tradesResult = await pool.query(
      `SELECT 'trade' as type, id, amount, symbol as asset, status, direction, result, created_at, NULL as completed_at
       FROM orders
       WHERE user_id = $1
       ORDER BY created_at DESC
       LIMIT 20`,
...
router.put('/user/:telegramId', adminCheck, async (req, res) => {
  try {
    const { telegramId } = req.params;
    const { balance_usdt, trade_mode, trading_blocked, needs_verification, verified, min_deposit, min_withdraw } = req.body;
