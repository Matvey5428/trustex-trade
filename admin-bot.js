...
// Admin bot token (create new bot via @BotFather)
...
bot.onText(/\/users/, async (msg) => {
  ...
  try {
    const result = await pool.query(`
      SELECT id, telegram_id, username, first_name, balance, mode, created_at 
      FROM users 
...
bot.onText(/\/user (.+)/, async (msg, match) => {
  ...
  try {
    ...
    const name = user.first_name || user.username || 'Без имени';
    const mode = user.mode === 'win' ? '🟢 WIN' : '🔴 LOSS';
...
bot.onText(/\/setmode (\S+) (win|loss)/, async (msg, match) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE users SET mode = $1 WHERE telegram_id = $2 RETURNING first_name, username',
      [newMode, searchId]
    );
...
bot.onText(/\/stats/, async (msg) => {
  ...
  try {
    const usersResult = await pool.query(`
      SELECT 
        COUNT(*) as total,
        COUNT(*) FILTER (WHERE mode = 'win') as win_mode,
        COUNT(*) FILTER (WHERE mode = 'loss') as loss_mode,
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('balance_')) {
    ...
    bot.sendMessage(chatId, 
      `💰 Введите новый баланс для пользователя:\n\n\`/setbalance ${telegramId} [сумма]\`\n\nНапример: \`/setbalance ${telegramId} 1000\``,
      { parse_mode: 'Markdown' }
    );
  }
});
